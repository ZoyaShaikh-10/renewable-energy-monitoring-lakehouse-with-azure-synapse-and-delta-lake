...
class EnergyProduction(WeatherLocationBase):
    ...
    def simulate_energy_production(self, weather_data, time_interval=24):
        ...
        for location_name, location_info in self.locations_data.items():
            ...
            ## Saving data for each location
            energy_production.save_to_csv(energy_data, f"realtime-output/{location_name}/", f"{location_name}_production_simulation.csv")
            
        return energy_data


    def save_to_csv(self, energy_data, location, output_file):
        """
        ...
        self.save_to_csvf(energy_data, location, output_file, fieldnames)
...
if __name__ == "__main__":
    ...
    with open("realtime-output/weather_forecast_multiple_locations.json", "r") as file:
